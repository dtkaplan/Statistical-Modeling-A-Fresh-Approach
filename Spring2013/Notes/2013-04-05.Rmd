April 5, 2013 Stats 155 Class Notes 
==================
...
* Range
* coverage intervals (esp. 95%)
* IQR
* sd and variance

Then we started build models.  Two basic quantities that we calculated:
* model coefficients --- an "effect size"
* R^2 --- about the relative size of the residual to the fitted value

Now we're translating into new quantities, ones that summarize the information in model coefficients and R^2 in different ways.
* model coefficients divided by the standard error give a t-statistic.
* the t-statistic gives a p-value.  Do problem 14.9.
* R^2 translates into an F-statistic given two additional pieces of information: degrees of freedom in the denominator and in the numerator.
* Model coefficients, the t statistic, and the p-value.
* R^2, the F statistic, and the p-value

Think of the p-value as a way of putting the various statistics --- model coefficients, R^2, t, F --- onto a common scale, where they are easily interpreted.

Remember that p doesn't carry all the information that's in the coefficient or R^2.  It's a summary that indicates the strength of evidence for rejecting the Null Hypothesis.
