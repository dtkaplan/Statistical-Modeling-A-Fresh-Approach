...
* p. 83 Introduction of `mm` operator for groupwise mean models. ```fitted()``` no longer works with ```mean()``` [New page](p83.pdf)
...
* p. 42: Use of `$` operator (deprecated in current `mosaic` version) [New page](p42.pdf)
* p. 43: Use of `transform()` to add new variables to a data frame [New page](p43.pdf)
...
* p. 196: `tally()` replaces `table()` [New page](p196.pdf)
* p. 236: Use of `with()` rather than `$`.  Fixed error in names of variables. [New page](p236.pdf)
