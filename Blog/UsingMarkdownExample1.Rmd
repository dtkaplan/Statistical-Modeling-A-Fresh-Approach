...
* Create the pool.  Why not do this explicitly, with `c()` being the only computer jargon?
```{r}
pool = c("F","F","F","F","F","F","F","F","F","F","M","M","M","M","M","M","M","M","M","M","M","M","M","M","M")
```
...
```{r}
trials = do(1000)*tally( resample(pool,8))
head(trials)
```
...
It looks like having 5 women promoted out of this pool is fairly likely to happen under the null hypothesis. Out of the 1000 trials, there were 5 or more women more than 15% of the time. So, there's no reason to reject the null hypothesis.

**NOTE TO MAVENS**:  This points out a feature of `tally()` that should be fixed: it gives NA rather than 0 when the count is 0.
