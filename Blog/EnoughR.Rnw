...
One key to success using \R\ in Intro Stats is keeping the volume low.  The colored 
commands below are sufficient for an Intro Stats course.  The others are optional extras.
This page provides a palette of functions.  The next page shows some examples.
\iffalse
In fact, \opt{many of these could be excluded}.
See \url{http://www.calvin.edu/~rpruim/courses/m143/S11/fromClass/} for notes used in an 
Intro Stats course for biology majors for usage examples.  
The commands below were culled from those notes.  Some commands are 
\fi
